...
*/
function getName(nameTag) {
  return 'Hello, my name is ' + nameTag.name;
}
...
*/
function makeSmartPerson(nameTag) {
  const smartPerson = {
    name: nameTag,
    ...
    speak: function() {
      return 'Hello, my name is ' + nameTag;
    } // end speak method
  }
  return smartPerson;
}
...
function getLastCarInfo(inventory) {
  const indexLength = inventory.length - 1;
  return 'This is a ' + inventory[indexLength].car_make + ' ' + inventory[indexLength].car_model;
}
...
function getGermanCars(inventory) {
  ...
  for (let i = 0; i < inventory.length; i++) {
    if (inventory[i].car_make.toUpperCase() === 'AUDI' ) {
      germanCar.push(inventory[i]);
    } else if (inventory[i].car_make.toUpperCase() === 'MERCEDES-BENZ') {
      germanCar.push(inventory[i]);
    } else if (inventory[i].car_make.toUpperCase() === 'VOLKSWAGEN') {
      germanCar.push(inventory[i]);
    } else if (inventory[i].car_make.toUpperCase() === 'BMW') {
      germanCar.push(inventory[i]);
    }
...
function carMaker(odoInput) {
  var odoCar = {
    odometer: odoInput,
    drive: function(distanceInput) {
      this.odometer += distanceInput;
